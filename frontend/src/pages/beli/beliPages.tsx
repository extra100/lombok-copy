...
const BeliPages: React.FC = () => {
  ...
  console.log({ selectedPelanganId })
  ...
  const handleJenisHargaChange = (hargaId: string, posId: string) => {
    const currentBeli = form.getFieldValue(posId)
  ...
  const handleRemove = (id: string) => {
    const updatedBelis = poss.filter((jos) => jos._id !== id)
  ...
  const hitungTotalSemua = (list = poss) => {
    let total = list.reduce((sum, pos) => {
      const nilaiTotalBeli = form.getFieldValue(pos._id)?.total || '0'
  ...
  const handleSave = () => {
    poss.forEach((pos) => {
      ...
      if (AmbilData) {
        updateBeliMutation.mutate({ id_beli, ...posDataToSave })
      } else {
        addBeliMutation.mutate(posDataToSave)
      }
      ...
      if (currentStok) {
        const oldQtySold =
          getBeliDetail?.find((detail) => detail._id === pos._id)?.qty_sold || 0
      ...
      render: (text: string, record: Beli) => (
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: `Please input Harga Beli!`,
            },
          ]}
          style={{ marginBottom: 0 }}
        >
                    ...
                    ? pelanggans
                        ...
                        .filter((combuned) => {
                          if (user && user.isAdmin) {
                            ...
                            return combuned.id_outlet === idOutletLoggedIn
                          }
                        })
                        .map((tag) => (
