...
interface KasPenjualanInterface {
  key: string
  name: string
  balance: number
  currency_id: string
...
export const useFetchKasPenjualan = () => {
  const [loading, setLoading] = useState(true)
  const [kasPenjuaanData, setKasPenjualanData] = useState<
    KasPenjualanInterface[]
  >([])
    ...
    const fetchData = async () => {
      try {
        ...
        const invoicesResponse = await fetch(
          `${HOST}/finance/bank?balances?page=1&per_page=1000000000`,
          {
            headers: {
        ...
        const formattedData: KasPenjualanInterface[] = data.data.map(
          (item: any) => ({
            key: item.id,
        ...
        setKasPenjualanData(formattedData)
        setLoading(false)
      } catch (error) {
  ...
  return { loading, kasPenjuaanData }
}
