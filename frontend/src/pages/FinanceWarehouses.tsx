...
export const useFinanceWarehouses = () => {
  const [loading, setLoading] = useState(true)
  const [financewarehouses, setfinancewarehouses] = useState<BankTransaction[]>(
    []
  )
    ...
    const fetchData = async () => {
      try {
        ...
        const bankTransResponse = await fetch(
          `${HOST}/finance/warehouses?page=1&per_page=1000`,
          {
            headers: {
        ...
        const { data } = await bankTransResponse.json()

        const formattedData: BankTransaction[] = data.data.map((item: any) => ({
          key: item.id,
          name: item.name,
          product_val: item.product_val,
          id: item.id,
        }))

        setfinancewarehouses(formattedData)
        setLoading(false)
      } catch (error) {
  ...
  return { loading, financewarehouses }
}
