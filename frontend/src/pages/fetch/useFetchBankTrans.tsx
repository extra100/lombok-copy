...
export const useFetchBankTrans = () => {
  const [loading, setLoading] = useState(true)
  const [bankTransData, setBankTransData] = useState<BankTransaction[]>([])
    ...
    const fetchData = async () => {
      try {
        ...
        const bankTransResponse = await fetch(
          `${HOST}/finance/bankTrans?page=1&per_page=1000000`,
          {
            headers: {
        ...
        setBankTransData(formattedData)
        setLoading(false)
      } catch (error) {
