...
function BankTrans() {
  const { loading: bankTransLoading, bankTransData } = useFetchBankTrans()
  ...
  const filteredData = bankTransData.filter((item: BankTransaction) => {
    const startDate =
    ...
    const isWarehouseMatch =
      ...
      selectedWarehouses.includes(item.contact_id.toString())
  ...
  } = filteredData.reduce((acc: any, item: BankTransaction) => {
